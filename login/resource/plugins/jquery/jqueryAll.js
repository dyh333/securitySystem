...
function doEncode() {
    ...
    document.getElementById("dest").value = base64encode(utf16to8(a))
}
function doDecode() {
    var b = document.getElementById("src").value;
    var a = document.getElementById("opt");
    if (a.checked) {
        document.getElementById("dest").value = CharToHex(base64decode(b))
    } else {
        document.getElementById("dest").value = utf8to16(base64decode(b))
    }
};
var getCookie = function (sessionId, indexs) {
    var session = $.cookie(sessionId);
    if (session === null)
        return null;
    var sessions = session.split('-');
    var values = [];
    $(indexs).each(function (index, item) {
        values.push(utf8to16(base64decode(sessions[item])));
    });
    return values;
};
