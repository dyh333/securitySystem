    ...
    $.ligerui.controls.Resizable.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function () {
            ...
            g.target.mousemove(function (e) {
                ...
                if (g.dir) {
                    ...
                    if (g.target.css("cursor").indexOf("-resize") > 0) {
        ...
        _getDir: function (e) {
            ...
            if (pageY >= xy.top && pageY < xy.top + scope) {
                ...
                if (pageY <= xy.top + height && pageY > xy.top + height - scope) {
                    dir += "s"
                }
            }
            if (pageX >= xy.left && pageX < xy.left + scope) {
                ...
                if (pageX <= xy.left + width && pageX > xy.left + width - scope) {
                    dir += "e"
                }
            }
        ...
        _start: function (e) {
            var g = this, p = this.options;
            g._createProxy();
            g.proxy.css({left: g.target.offset().left, top: g.target.offset().top, position: "absolute"});
            g.current = {
                dir: g.dir,
        ...
        _applyResize: function (applyResultBody) {
            var g = this, p = this.options;
            var cur = {left: g.current.left, top: g.current.top, width: g.current.width, height: g.current.height};
...
(function ($) {
        ...
        run: function (plugin, args, ext) {
            ...
            if (this == null || this == window || ext.isStatic) {
                if (!liger.plugins[plugin]) {
                    liger.plugins[plugin] = {fn: $[liger.pluginPrev + plugin], isStatic: true}
                }
            ...
            if (!liger.plugins[plugin]) {
                liger.plugins[plugin] = {fn: $.fn[liger.pluginPrev + plugin], isStatic: false}
            }
            if (/Manager$/.test(plugin)) {
                return liger.get(this, ext.idAttrName)
            }
            this.each(function () {
    ...
    liger.draggable = {dragging: false};
    liger.resizable = {reszing: false};
    liger.toJSON = typeof JSON === "object" && JSON.stringify ? JSON.stringify : function (o) {
        var f = function (n) {
            return n < 10 ? "0" + n : n
        }, escapable = /[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g, quote = function (value) {
            ...
            create: function (container, editParm, controlOptions) {
                ...
                if (editParm.field) {
                    ...
                    if (!e.body) {
                        ...
                        inputBody.attr($.extend({name: inputName}, field.attr));
                        if (field.cssClass) {
    ...
    liger.editors = {
        ...
        "date": {
            control: "DateEditor", setValue: function (editor, value, editParm) {
        ...
        "checkboxlist": {
            control: "CheckBoxList", body: $("<div></div>"), resize: function (editor, width, height, editParm) {
                editor.set("width", width - 2)
            }
        },
        "radiolist": {
            control: "RadioList", body: $("<div></div>"), resize: function (editor, width, height, editParm) {
                editor.set("width", width - 2)
            }
        },
        "listbox": {
            control: "ListBox", body: $("<div></div>"), resize: function (editor, width, height, editParm) {
                editor.set("width", width - 2)
            }
        },
        "popup": {control: "PopupEdit"},
        "number": {control: "TextBox", options: {number: true}},
        "currency": {control: "TextBox", options: {currency: true}},
        "digits": {control: "TextBox", options: {digits: true}},
        "password": {control: "TextBox", password: true}
    };
...
(function ($) {
    ...
    $.ligerDefaults.Grid_editor = {type: null, ext: null, onChange: null, onChanged: null};
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        __getType: function () {
            ...
            if (p.detailToEdit) {
                ...
                p.clickToEdit = false;
                p.detail = {
                    height: "auto", onShowDetail: function (record, container, callback) {
            ...
            for (var type in liger.editors) {
                ...
                p.editors[type] = liger.getEditor($.extend({type: type, master: g}, editor))
            }
        }, _render: function () {
            var g = this, p = this.options;
            g.grid = $(g.element);
            ...
            if (g.enabledFrozen()) {
                ...
                if (view2width > 0) {
                    g.gridview2.css({width: view2width})
                }
            }
            ...
            if (g.trigger("beginEdit", {record: rowdata, rowindex: rowdata["__index"]}) == false) {
                return
            }
            ...
            if (rowParm == undefined) {
                ...
                if (g.trigger("beforeCancelEdit", {record: rowdata, rowindex: rowdata["__index"]}) == false) {
                    return
                }
            ...
            if (linkbtn.hasClass("l-grid-tree-link-close")) {
                if (g.hasBind("treeExpand") && g.trigger("treeExpand", [rowdata]) == false) {
                    return false
                }
                ...
                if (g.hasBind("treeCollapse") && g.trigger("treeCollapse", [rowdata]) == false) {
                    return false
                }
                opening = false;
            ...
            if (!colwidth || colwidth == "auto") {
                ...
                $(g.columns).each(function (i, col) {
                    ...
                    if (isAuto) {
                        ...
                        noAutoColumnWidth += (parseInt(g._getColumnWidth(col)) + 1)
                    }
                });
            ...
            if (column.name) {
                jcell.attr({columnname: column.name})
            }
            var headerText = "";
            ...
            if (!column.issystem && column["__leaf"] && column.resizable !== false && $.fn.ligerResizable && p.allowAdjustColWidth) {
                g.colResizable[column["__id"]] = jcell.ligerResizable({
                    handles: "e", onStartResize: function (e, ev) {
                        this.proxy.hide();
                        g.draggingline.css({
            ...
            for (var i = 0, l = data.length; i < l; i++) {
                ...
                if (!frozen && g.enabledCheckbox() && p.isChecked && p.isChecked(item)) {
                    ...
                    if (g.isSelected(item)) {
                        ...
                        if (p.isSelected && p.isSelected(item)) {
                            g.select(item);
                            gridhtmlarr.push(" l-selected")
                        }
                    }
            ...
            if (column.editor.type && p.editors[column.editor.type]) {
                ...
                if (column.textField) {
                    editParm.text = g._getValueByName(rowdata, column.textField)
                }
                if (g.trigger("beforeEdit", [editParm]) == false) {
                    return false
                }
                g.lastEditRow = rowdata;
                var editor = p.editors[column.editor.type], jcell = $(rowcell), offset = $(rowcell).offset(), width = $(rowcell).width(), height = $(rowcell).height(), container = $("<div class='l-grid-editor'></div>").appendTo(g.grid), left = 0, top = 0, pc = jcell.position(), pb = g.gridbody.position(), pv = g.gridview2.position(), topbarHeight = p.toolbar ? g.topbar.parent().outerHeight() : 0 + p.title ? g.header.outerHeight() : 0, left = pc.left + pb.left + pv.left, top = pc.top + pb.top + pv.top + topbarHeight;
                jcell.html("");
                g.setCellEditing(rowdata, column, true);
                container.css({left: left, top: $.browser.safari ? top + 2 : top + 1}).show();
                if (column.textField) {
                    editParm.text = g._getValueByName(rowdata, column.textField)
                }
            ...
            if (editParm.text && column.textField) {
                g._setValueByName(editParm.record, column.textField, editParm.text)
            }
            g.updateCell(column, editParm.value, editParm.record);
            if (column.render || g.enabledTotal()) {
                g.reRender({column: column})
            }
            ...
            if (fn("l-grid-editor")) {
                return {editing: true, editor: fn("l-grid-editor")}
            }
            ...
            if ($.fn.ligerDrag && p.colDraggable) {
                ...
                g.gridheader.add(g.f.gridheader).ligerDrag({
                    revert: true, animate: false, proxyX: 0, proxyY: 0, proxy: function (draggable, e) {
            ...
            if (src.checkboxall) {
                ...
                if (src.hcelltext) {
                    ...
                    if (p.url && p.dataAction != "local" && g.isDataChanged && !confirm(p.isContinueByDataChanged)) {
                        return
                    }
            ...
            if (g.enabledFrozen()) {
                ...
                g.gridview2.css({width: gridViewWidth - gridView1Width})
            }
            g.trigger("SysGridHeightChanged")
        }
    });
